...
export const personSchema = z
  .object({
    ...
    dob: z
      ...
      .refine((date) => date <= eighteenYearsAgo, {
        message: "You must be at least 18 years old to signup",
      })
      .refine((date) => date >= hundredYearsAgo, {
  ...
  .refine(
    ...
    },
    {
      ...
      message:
        "A us citizen has to provide either driving license or state id and for non citizen password and visa is required",
    },
  )
  .refine(
    (data) => {
  ...
  .refine(
    ...
    },
    {
      path: ["no_ssn"],
      message:
        "If SSN is not provided, a valid reason for not having a SSN must be provided",
    },
  )
  .refine(
    (data) => {
